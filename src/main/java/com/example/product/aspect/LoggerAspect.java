...
import com.example.product.exception.NosSearchProductException;
...
public class LoggerAspect {
    ...
    public Object aroundControllerMethodExecution(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            return result;
        } catch (NosSearchProductException ex) {
            logger.error("Exception in method " + methodName + ": " + ex.getMessage());
            throw ex;
        }
    }

    @AfterThrowing(pointcut = "execution(* com.example.product.service.*.*(..))", throwing = "ex")
